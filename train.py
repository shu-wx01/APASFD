...
from utils import set_logger, get_current_ts, get_args, set_random_seeds, GradualWarmupScheduler
...
def train(args, logger):
    ...
    train_loader, _, test_loader, _, num_test_samples = dataloader(args, g)
    ...
    for epoch in range(args.n_epoch):
        ...
        for batch_idx, (input_nodes, pos_graph, neg_graph, blocks, frontier, current_ts) in enumerate(train_loader):
            ...
            if batch_idx != 0:
                ...
                logits, labels, nodeEmb = decoder(emb, pos_graph, neg_graph)

                labels_all.extend(labels)
                nodeEmb_all.extend(nodeEmb)
        ...
        # val_auc, val_acc, val_loss = eval_epoch(args, logger, g, val_loader, encoder, decoder, msg2mail, loss_fcn, device, num_val_samples)
        # logger.info('Val {} Task | auc: {:.4f} | acc: {:.4f} | Loss: {:.4f}'.format(args.tasks, val_auc, val_acc, val_loss))
        ...
        test_auc, test_acc, test_loss = eval_epoch(args, logger, g, test_loader, encoder, decoder, msg2mail, loss_fcn, device, num_test_samples)
        test_msg = 'Test {} Task | epoch: {} | auc: {:.4f} | acc: {:.4f} | Loss: {:.4f}'.format(args.tasks, epoch, test_auc, test_acc, test_loss)
        ...
        if test_auc > best_auc:
            ...
            best_auc = test_auc
            best_acc = test_acc
            logger.info(f'Saving the best model at epoch {best_epoch}')
            torch.save(encoder.state_dict(), MODEL_SAVE_PATH+current_time+get_model_name('Encoder'))
            torch.save(decoder.state_dict(), MODEL_SAVE_PATH+current_time+get_model_name('Decoder'))
